...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        int searchChildren = numLessThanEqual(key,keys);
        BPlusNode stealKids = getChild(searchChildren);
        LeafNode findKid = stealKids.get(key);
        return findKid;
    }
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        // TODO(proj2): implement
        // Note B+ trees are sorted by smallest to largest, so by definition we know first element
        // is the smallest and last is the largest example: 1 2 3 4
        BPlusNode smallestLeaf = getChild(0);
        LeafNode leftmostLeaf = smallestLeaf.getLeftmostLeaf();
        return leftmostLeaf;
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        /**
         * Only the leaf nodes contain records (or pointers to records - this will be explained later).
         * The inner nodes (which are the non-leaf nodes) do not contain the actual records
         *
         * Note Inner Nodes are simply indexes, or sites to our leaf nodes, they dont contain records
         */
        int d = metadata.getOrder();
        int maxEntries = 2*d;
        int removeFirst = 1;
        int findIndex = InnerNode.numLessThanEqual(key,this.keys);
        BPlusNode branchToIndex = getChild(findIndex);
        Optional<Pair<DataBox, Long>> inIndex = branchToIndex.put(key, rid);
        if (!inIndex.isPresent()) { //Case 1:Key already exists
            //throw new BPlusTreeException("Key already exists.");
            return Optional.empty();
        }
        DataBox firstItem = inIndex.get().getFirst();
        Long secondItem = inIndex.get().getSecond();
        this.keys.add(findIndex, inIndex.get().getFirst()); /*Gets first value of pair*/
        this.children.add(findIndex+1,inIndex.get().getSecond()); /* Gets second*/
        if (keys.size() <= maxEntries) { //Case 2: No Overflow
            sync();
            return Optional.empty();
        }
        if ((this.keys.size() > maxEntries)) {
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();
            DataBox getMiddle = this.keys.get(d);
            for (int i = d; i <= maxEntries; i++) { // 1 2 3 4 5 =  1 2 4 5
                DataBox secondHalfKeys = this.keys.remove(d);
                keys.add(secondHalfKeys);
                removeFirst -= 1;
            }
            for (int i = d; i <= maxEntries; i++) {
                Long secondHalfLeaf = this.children.remove(d+1); /*When split, remove d+1*/
                children.add(secondHalfLeaf);
            }
            keys.remove(0); /*Move entries in inner node, NOT COPY*/
            InnerNode rightNode = new InnerNode(metadata, bufferManager, keys, children, treeContext);
            Page rightPage = rightNode.getPage();
            long pageNumber = rightPage.getPageNum();
            //this.rightSibling = Optional.of(pageNumber);
            sync();
            return Optional.of(new Pair<DataBox,Long>(getMiddle,pageNumber));
        }
        sync();
        return Optional.empty();
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        //For self: When node fills up
        // For inner node max ceiling hint is 2*d+1
        //check if pair exists
        //Split node how u would a normal put
        if (fillFactor > 1 || fillFactor < 0) {
            throw new BPlusTreeException("Fill Factor cannot be negative or greater than 1");
        }
        int d = metadata.getOrder();
        int maxEntries = 2*d;
        int removeFirst = 1;
        int maxCeiling = 2*d+1;
        while (data.hasNext() && this.keys.size() < maxCeiling) {
            BPlusNode getRightChild = getChild(this.keys.size());
            Optional<Pair<DataBox, Long>> newPair = getRightChild.bulkLoad(data,fillFactor);
            if (newPair.isPresent()) {
                this.keys.add(newPair.get().getFirst());
                this.children.add(newPair.get().getSecond());
            }
        }
        if (this.keys.size() <= maxCeiling) { //Case 2: No Overflow
            sync();
            return Optional.empty();
        }
        DataBox returnMiddle = this.keys.get(d);
        if (this.keys.size() > maxCeiling) { //Case 3: Return a pair of split key and right node
            List<DataBox> keys = new ArrayList<>();
            List<Long> children = new ArrayList<>();
            for (int i = d; i <= maxEntries; i+=1) { // 1 2 3 4 5 =  1 2 4 5
                DataBox secondHalfKeys = this.keys.remove(d);
                keys.add(secondHalfKeys);
                removeFirst -= 1;
            }
            for (int i = d; i <= maxEntries; i+=1) {
                Long secondHalfLeaf = this.children.remove(d); /*When split, remove d+1*/
                children.add(secondHalfLeaf);
            }
            keys.remove(0); /*Dont copy key inner node*/
            InnerNode rightNode = new InnerNode(metadata, bufferManager, keys, children, treeContext);
            Page rightPage = rightNode.getPage();
            long pageNumber = rightPage.getPageNum();
            sync();
            return Optional.of(new Pair<DataBox,Long>(returnMiddle,pageNumber));
        }
        sync();
        return Optional.empty();
    }
    ...
    public void remove(DataBox key) {
        // TODO(proj2): implement
        int currentInner = InnerNode.numLessThanEqual(key, keys);
        BPlusNode child = getChild(currentInner);
        child.remove(key);
    }
