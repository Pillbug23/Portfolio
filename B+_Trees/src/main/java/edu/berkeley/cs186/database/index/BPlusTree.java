...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        Optional<RecordId> obtainKey = start.getKey(key);
        return obtainKey;
    }
    ...
    public Iterator<RecordId> scanAll() {
        ...
        //LeafNode startingRoot = this.root.get(key);
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
        LeafNode startingRoot = this.root.get(key);
        return new BPlusTreeIterator(startingRoot, Optional.of(key));
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> putValue = root.put(key, rid);
        ...
        if (root.get(key).getRids().equals(rid)) {
            System.out.println("Root Node already exists.");
            return;
        }
        if (putValue == null) {
        ...
        Pair<DataBox, Long> obtainValue = putValue.get();

        DataBox firstValue = obtainValue.getFirst();
        Long secondValue = obtainValue.getSecond();
        keys.add(firstValue);

        Page obtainPage = root.getPage(); //*.getPage.getPagenum
        long obtainPage2 = obtainPage.getPageNum();
        children.add(obtainPage2);
        children.add(secondValue); /*Children will be old root*/

        InnerNode createInner = new InnerNode(metadata, bufferManager, keys, children, lockContext);
        updateRoot(createInner); /*New root node*/
        return;
    }
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        /*int d = metadata.getOrder();
        int maxOrder = 2 * d;
        //only error check is that the tree must be empty + no need to check in other files.
         */
        ...
        if ((root instanceof LeafNode) && ((LeafNode) root).getKeys().size() == 0) {
            while (data.hasNext()) {
                Optional<Pair<DataBox, Long>> putValue = root.bulkLoad(data, fillFactor);
                List<DataBox> keys = new ArrayList<>();
                List<Long> children = new ArrayList<>();
                int height = 0;

                if (putValue.isPresent()) { /*Check if node is empty*/
                    height += 1;
                    Pair<DataBox, Long> obtainValue = putValue.get();
                    DataBox firstValue = obtainValue.getFirst();
                    ...
                    Long secondValue = obtainValue.getSecond(); //long instead of Long :doesnt matter

                    Page obtainPage = this.root.getPage(); //*.getPage.getPagenum
                    Long obtainPage2 = obtainPage.getPageNum();

                    children.add(obtainPage2);
                    children.add(secondValue);

                    //this it uppp
                    //this.root = new InnerNode(this.metadata, this.bufferManager, keys, children, this.lockContext);
                    InnerNode createInner = new InnerNode(metadata, bufferManager, keys, children, lockContext);
                    updateRoot(createInner);
                }
            }
        }
        else {
            String msg = "The tree is non empty!";
            throw new BPlusTreeException(msg);
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private LeafNode startingRoot;
        private Iterator<RecordId> iterateTree;

        public BPlusTreeIterator(BPlusNode n, Optional<DataBox> datakey) {
            this.startingRoot = n.getLeftmostLeaf();
            if (!datakey.isPresent()) {
                this.iterateTree = this.startingRoot.scanAll();
            }
            else {
                this.iterateTree = this.startingRoot.scanGreaterEqual(datakey.get());
            }
        }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            //if (!startingRoot.getRightSibling().isPresent()) {
            //    return false;
            //}

            //return iterateTree.hasNext();
            if (!this.iterateTree.hasNext()) {
                Optional<LeafNode> nextNode = this.startingRoot.getRightSibling();
                if (nextNode.isPresent()) {
                    this.startingRoot = nextNode.get();
                    //must check
                    //printf(startingRoot);
                    this.iterateTree = this.startingRoot.scanAll();
                    //return this.hasNext();
