...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while current_place != beehive:
            if current_place.bees and self.min_range<=range<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        list1_copy = self.place.bees[:] 
        Ant.reduce_armor(self,amount) #Case 0:Fire ant takes dmg
        #alternate solution proposed by comments
        for x in list1_copy:
            if self.armor > 0:
                x.reduce_armor(amount)
            if self.armor <= 0:
                x.reduce_armor(self.damage + amount)
        #alternate solution proposed by comments
