...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        // to get a hash value.
        int whichColumn = getLeftColumnIndex();
        if (left) {
            whichColumn = getLeftColumnIndex();
        } else {
            whichColumn = getRightColumnIndex();
        }
        for (Record record :records) {
            DataBox nextRec = record.getValue(whichColumn);
            int hasValue = HashFunc.hashDataBox(nextRec, pass);
            int partitionNum = (hasValue % partitions.length);
    ...
    private void run(Iterable<Record> leftRecords, Iterable<Record> rightRecords, int pass) {
        ...
        for (int i = 0; i < leftPartitions.length; i++) {
            ...
            if (leftPartitions[i].getNumPages() <= numBuffers - 2 || rightPartitions[i].getNumPages() <= numBuffers ) {
