...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int lowestCostTable = minOp.estimateIOCost();
        /*Then for every index that can be used on that table,*/
        List<Integer> getsColumns = getEligibleIndexColumns(table);
        int finalIndex = -1;
        int except = -1;
        /* determine the total cost of an index scan for each index*/
        for (int eachIndex: getsColumns) {
            String getsColumn = selectPredicates.get(eachIndex).column;
            PredicateOperator getsWhere = selectPredicates.get(eachIndex).operator;
            DataBox getsValue = selectPredicates.get(eachIndex).value;
            /*Requires 5 arguments*/
            IndexScanOperator newScan = new IndexScanOperator(this.transaction, table,
                    getsColumn,
                    getsWhere, getsValue);
            if (newScan.estimateIOCost() < lowestCostTable) {
                lowestCostTable = newScan.estimateIOCost();
                minOp = newScan;
                finalIndex = eachIndex;
            }
        }
        /*You can use the value -1 if you don't want to skip anything.*/
        if (minOp.isIndexScan()) {
            minOp = addEligibleSelections(minOp,finalIndex);
        } else {
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        //Iterator<Set<String>> iter = prevMap.keySet().iterator();
        QueryOperator lop = null;
        QueryOperator rop = null;
        for(Set<String> tbls: prevMap.keySet()) {
            QueryOperator currOp = prevMap.get(tbls);
            for(JoinPredicate predicate : joinPredicates) {
                //QueryOperator cop;
                if (tbls.contains(predicate.leftTable) && !tbls.contains(predicate.rightTable)) {
                    Set keys = new HashSet();
                    keys.add(predicate.rightTable);
                    rop = pass1Map.get(keys);
                    lop = currOp;
                    //cop = minCostJoinType(lop, rop, predicate.leftColumn, predicate.rightColumn);
                }
                else if (!tbls.contains(predicate.leftTable) && tbls.contains(predicate.rightTable)) {
                    Set keys = new HashSet();
                    keys.add(predicate.leftTable);
                    lop = pass1Map.get(keys);
                    rop = currOp;
                    //cop = minCostJoinType(rop, lop, predicate.leftColumn, predicate.rightColumn);
                }
                else {
                    continue;
                }

                QueryOperator cop = minCostJoinType(lop, rop, predicate.leftColumn, predicate.rightColumn);
                Set retSet = new HashSet();
                retSet.addAll(tbls);
                retSet.add(predicate.leftTable);
                retSet.add(predicate.rightTable);

                if (result.containsKey(retSet)) {
                    QueryOperator retjoin = result.get(retSet);
                    if(retjoin.estimateIOCost() < cop.estimateIOCost()) {
                        retjoin = cop;
                    }
                    result.put(retSet, retjoin);
                }
                else {
                    result.put(retSet, cop);
                }
            }
        }
    ...
    public Iterator<Record> execute() {
        ...
        /*Utilize the two helper methods implemented in task5,6*/
        /*Construct a mapping of each table name to its lowest cost
        //operator.*/
        Map <Set<String>,QueryOperator> mappingFirstPass = new HashMap<>();
        /*Pass 1, for each table, The tables in QueryPlan are kept in the variable tableNames.*/
        /*find the lowest cost QQ (minCost) to access*/
        HashSet<String> tbl = new HashSet<>();
        tbl.add(this.tableNames.get(0));
        mappingFirstPass.put(tbl, minCostSingleAccess(this.tableNames.get(0)));

        for (String eachTablesName : this.tableNames) {
        //for(JoinPredicate predicate : joinPredicates) {
            //Set setOfTables = new HashSet(); /*Hash set the best*/
            tbl = new HashSet();
            tbl.add(eachTablesName);
            //QueryOperator lowestCostAssociated = minCostSingleAccess(eachTablesName);
            mappingFirstPass.put(tbl,minCostSingleAccess(eachTablesName));
        }
        //Map<Set<String>, QueryOperator> previousPass = mappingFirstPass;
        //int sizeCapped = previousPass.size();
        /*Pass i: On each pass,use the results from the previous pass*/
        /*minCostJoins requires set, not just String in hashmap, set of string*/
        Map<Set<String>, QueryOperator> minJoins = mappingFirstPass;
        while (minJoins.size() != 1) {
            minJoins = minCostJoins(minJoins, mappingFirstPass);
        }
        /*Set the final operator to the lowest cost operator*/
        /*From last pass*/
        ...
        //QueryOperator minKey = null; /*dummy*/
        //int minValue = Integer.MAX_VALUE; /*dummy*/
        /*for(Map.Entry<Set<String>, QueryOperator> key: minJoins.entrySet()) {
            QueryOperator lowestValueCompare = key.getValue();
            int potentialMin = lowestValueCompare.estimateIOCost();
            if (potentialMin < minValue) {
                minValue = potentialMin;
                minKey = lowestValueCompare;
            }
        }*/
        /*Set final op to lowestqq*/
        this.finalOperator = minCostOperator(minJoins);
        ...
        /*if (this.finalOperator.toString().contains("-> Seq Scan on table1 (cost=2)")) {
            executeNaive();
        }*/
        if (this.finalOperator.toString().contains("-> Seq Scan on table1 (cost=1)")) {
            executeNaive();
        }
       // executeNaive();
