...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        while(records.hasNext()) {
            temp.add(records.next());
        }
        //temp.sort(SortOperator.this.comparator);
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        // TODO(proj3_part1): implement
        PriorityQueue<Pair<Record, Integer>> prior = new PriorityQueue(runs.size(), new RecordPairComparator());
        List<Iterator<Record>> iter_list = new ArrayList<>();
        Run ret = makeRun();

        for(int i=0; i<runs.size(); i++) {
            Iterator<Record> iter_per_run = runs.get(i).iterator();
            Record r = iter_per_run.next();
            Pair<Record, Integer> p = new Pair(r, i);
            iter_list.add(iter_per_run);
            prior.add(p);
        }

        //now we are done w filling.
        while(prior.isEmpty() == false) {
            Pair<Record, Integer> p_next = prior.poll();
            int i = p_next.getSecond();
            ret.add(p_next.getFirst());

            //need to add first pair vals to ret
            Iterator<Record> curr_iter = iter_list.get(i);

            if(curr_iter.hasNext() == true) {
                Record r = curr_iter.next();
                Pair<Record, Integer> p = new Pair(r, i);
                prior.add(p);
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for(int i=0; i<runs.size(); i+=j) {
            List<Run> temp = runs.subList(i, i+j);
            ret.add(mergeSortedRuns(temp));
        }
    ...
    public Run sort() {
        ...
        while(sourceIterator.hasNext()) {
            BacktrackingIterator<Record> block = QueryOperator.getBlockIterator(sourceIterator, outputSchema, numBuffers);

            ret.add(sortRun(block));
        }
        while(ret.size()>1) {
            ret = mergePass(ret);
        }
