...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        apply = scheme_apply(procedure, args, env)
    return apply
        # END PROBLEM 4
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if self.parent != None:
            lookup_parent = self.parent
            return Frame.lookup(lookup_parent, symbol)
        # END PROBLEM 2
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals != nil and vals != nil:
            ...
            if formals.rest == nil or vals.rest == nil:
                return child_frame
            if formals.rest !=nil or vals.rest != nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        var_value = scheme_eval(expressions.rest.first, env)
        env.define(target,var_value)
        return target
        # END PROBLEM 5
        ...
        create_lambda_procedure = do_lambda_form(Pair(formals, body),env)
        env.define(name,create_lambda_procedure)
        return name
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions.rest == nil:
        return scheme_eval(expressions.first, env)
    if expressions != nil:
        evaluation = scheme_eval(expressions.first, env)
        if is_false_primitive(evaluation):
            return evaluation
        else:
...
def do_or_form(expressions, env):
    ...
    if expressions.rest == nil:
        return scheme_eval(expressions.first, env)
    if expressions != nil:
        evaluation2 = scheme_eval(expressions.first, env)
        if is_true_primitive(evaluation2):
            return evaluation2
        else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while bindings != nil:
        first_name_bind = bindings.first
        validate_form(first_name_bind, 2, 2)
        names = Pair(first_name_bind.first, names)
        evaluate = scheme_eval(first_name_bind.rest.first, env)
        values = Pair(evaluate, values)
