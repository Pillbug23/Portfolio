...
public class TETile implements Serializable {
    ...
    //private final String descriptionSpanish;
    ...
    public TETile(char character, Color textColor, Color backgroundColor, String description,
                  String filepath /*, String descriptionSpanish*/) {
     ...
     * @param description The description of the tile, shown in the GUI on hovering over the tile.
     */
    public TETile(char character, Color textColor, Color backgroundColor, String description/*, String descriptionSpanish*/) {
        this.character = character;
        this.textColor = textColor;
        this.backgroundColor = backgroundColor;
        this.description = description;
     ...
     * // textColor foreground color for tile copy
    ...
    public TETile(TETile t, Color backgroundColor) {
        this(t.character, t.textColor, backgroundColor, t.description/*, t.descriptionSpanish*/, t.filepath);
    }


    //False for background color, true for foreground
    /*public TETile(TETile t, Color tileColor, Boolean type) {
        if (type == true){
            this(t.character, tileColor, t.backgroundColor, t.description, t.filepath);

        } else {

            this(t.character, t.textColor, backgroundColor, t.description, t.filepath);
        }
    ...
    public static TETile colorDecrement(TETile t, int dr, int dg, int db){
        //Color oldColor = t.textColor;
        Color oldBack = t.backgroundColor;

        /*
        int newRed = Math.min(255, Math.max(0, oldColor.getRed() - dr));
        int newGreen = Math.min(255, Math.max(0, oldColor.getGreen() - dg));
        int newBlue = Math.min(255, Math.max(0, oldColor.getBlue() - db));

         */

        int newRedd = Math.min(255, Math.max(0, oldBack.getRed() + dr));
        int newGreenn = Math.min(255, Math.max(0, oldBack.getGreen() + dg));
        int newBluee = Math.min(255, Math.max(0, oldBack.getBlue() + db));

        Color d = new Color(newRedd, newGreenn, newBluee);
        //Color c = new Color(newRed, newGreen, newBlue);
        return new TETile(t, d);
    }
